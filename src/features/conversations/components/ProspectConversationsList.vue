  ...
  <aside class="prospect-list h-full flex flex-col bg-gray-900 border-r border-purple-900/20">
    ...
    <div class="p-3 flex-1 overflow-auto">
      ...
      <div v-else>
        ...
        <ul class="space-y-2">
          <li v-for="conv in filtered" :key="conv._id" @click="select(conv)" class="p-2 rounded-lg cursor-pointer flex items-center gap-3 hover:bg-gray-800">
            ...
            <div class="text-xs text-gray-400">{{ conv.messages?.length ?? 0 }}</div>
          </li>
        </ul>
      </div>
    </div>
...
interface Message { role?: string; content?: string; timestamp?: string | number }
interface Conversation { _id: string; prospectName: string; company?: string; messages?: Message[]; lastUpdated?: string | number }
...
function avatar(conv: Conversation) {
  return conv.prospectName ? conv.prospectName.charAt(0).toUpperCase() : '?'
}
        ...
        <div v-else>
          ...
          <ul class="space-y-2">
            <li
              ...
              :key="conv._id"
              @click="select(conv)"
              :class="['p-2 rounded-lg cursor-pointer flex items-center gap-3', selectedId === conv._id ? 'bg-purple-800/30 border border-purple-700' : 'hover:bg-gray-800']"
            >
              <div class="w-10 h-10 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white font-semibold">{{ avatar(conv) }}</div>
              <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold text-white truncate">{{ conv.prospectName }}</div>
...
const filtered = computed(() => {
  const q = query.value.trim().toLowerCase()
  const list = [...conversations.value].sort((a, b) => new Date(String(b.lastUpdated)).getTime() - new Date(String(a.lastUpdated)).getTime())
  return q ? list.filter(c => c.prospectName.toLowerCase().includes(q) || (c.company || '').toLowerCase().includes(q)) : list
})
